...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($company_name) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO employers (company_name, email, password) VALUES (?, ?, ?)");
            $stmt->execute([$company_name, $email, $hashed_password]);
            $success = "Registration successful! Redirecting to login...";
            // Redirect after 2 seconds
            header("Refresh:2;url=login.php");
            $_SESSION['company_name'] = $company_name; // Optional: Set session for welcome message
...
<head>
    ...
    <title>Register - Employer | Job Portal</title>
        ...
        <div class="container">
            ...
            <form class="register-form" method="POST">
                <div class="form-group">
                    ...
                    <input type="text" class="input-field" id="company_name" name="company_name" placeholder="Enter company name" value="<?php echo isset($company_name) ? htmlspecialchars($company_name) : ''; ?>" required>
                </div>
                <div class="form-group">
